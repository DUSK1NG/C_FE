...
这是一个无服务器、无第三方依赖的静态网页，用来编辑、导入、校验、预览、分析和导出二维桁架有限元 `.model` 文件。它与现有 C11 求解器使用同一种输入格式，但不会修改或直接调用 C 源代码。
...
3. 查看“当前状态”和 `.model` 预览；存在错误时“开始分析”和导出按钮会被禁用。
4. 点击“开始分析”，在“分析结果”面板查看节点位移、单元结果、支座反力、平衡汇总、节点与杆件变形图以及单元轴力图。
5. 在“导出文件名”中输入名称，点击“导出 .model”。浏览器会把规范化后的文本文件下载到浏览器配置的下载位置。
6. 如需 C11 CLI 的 TXT、Markdown、CSV 输出，再将导出的文件用于终端中的 `fem --input ...` 命令。

“开始分析”完全在本机浏览器的 JavaScript 中完成，不需要服务器、Docker、Node.js 或网络连接；网页也不能启动 `fem`/`fem.exe`。C11 CLI 仍可用于参考和生成 TXT、Markdown、CSV 文件，但与浏览器端分析是两条独立路径。

网页编辑器不包含后端服务，也不会修改仓库中的源文件。页面下载的模型文件由浏览器决定保存位置；请在运行命令前确认 `--input` 指向该文件。
...
浏览器端 JavaScript 分析和 C11 CLI 都不做单位换算，整个模型必须使用一致单位制。例如使用 N–mm 制时，坐标用 mm、面积用 mm²、`E` 用 N/mm²（MPa）、荷载用 N；不要在同一模型中混用 m、mm、N、kN 等单位。
...
- 无法解析的导入不会覆盖页面中已有模型；错误会显示在状态区域，文件选择框会重置以便重新选择同一文件。
...
当前版本只是通过本地文件打开的静态页面。出于浏览器安全边界，它不能直接启动仓库中的 `fem`/`fem.exe`，也不能替用户创建结果目录或写入 TXT、Markdown、CSV 求解结果。浏览器端 JavaScript 只显示网页中的分析表格和图形；页面中的 CLI 命令仅是终端操作提示，请复制或按平台调整后，在仓库根目录手动运行。
