...
- 已完成 Stage 1–10，并保留统一命令行入口 `fem`。
- 新增本地网页模型编辑器：可加载示例、导入 `.model`、编辑四类数据、校验、预览和导出。
- 网页编辑器是纯静态页面，不需要 Docker、Node.js、npm 或网络连接。
- 网页只负责生成模型文件；有限元求解仍在终端中运行 `fem` 或 `fem.exe`。
- GitHub `main` 分支包含网页编辑器、中文说明和完整回归测试。
...
网页模型编辑器的独立测试可使用工作区提供的 Node.js 运行时执行：

```bash
node tests/test_web_model.js
```

完整验证覆盖 Stage 1–10、统一管线、输出选择、CLI 解析和网页模型逻辑。浏览器无法直接启动 `fem`，请将网页导出的 `.model` 文件交给终端命令处理。
