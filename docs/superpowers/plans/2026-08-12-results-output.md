...
**Goal:** Add a fixed-capacity TXT/CSV results exporter and `DEBUG`-controlled matrix/vector printers without changing the Stage 1–8 solver interfaces or the existing Demo entry point.

**Architecture:** Add an `output` module with a public `FemResults` container and four output functions. The module consumes the existing `FemModel` and `ElementResult` structures, validates only the data it serializes, and writes deterministic human-readable or machine-readable output. Debug printers remain side-effect-only console helpers; callers decide whether to invoke them with the existing `DEBUG` macro.
...
- TXT numeric fields use `%.12g`; CSV uses the exact header and `record_type` values from the approved design.
...
**Interfaces:**
- The test defines the required public API by including `output.h` and calling `write_results_txt`, `write_results_csv`, `print_debug_matrix`, and `print_debug_vector`.
...
### Task 2: Implement TXT/CSV export and Debug printers
...
**Interfaces:**
- `output.h` includes `config.h`, `fem.h`, `io.h`, and `postprocess.h` and exposes the approved `FemResults` type and four functions.
- `write_results_txt` and `write_results_csv` accept a path, a `FemModel`, and a `FemResults`; successful writes return `FEM_OK`.
...
- [ ] **Step 4: Implement Debug printers**
...
- [ ] **Step 5: Run Stage9 tests to verify GREEN**
...
- [ ] Stage9 TXT and CSV formats match the approved design exactly.
